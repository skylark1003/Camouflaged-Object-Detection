...
# 使用ResNet和注意力机制来进行图像分割任务，同时还有生成器和分类器。

# Generator是一个简单的模型，它接收一个输入图像并返回三个输出，即fix_pred、cod_pred1和cod_pred2。
# 这三个输出都是通过Saliency_feat_encoder（SFE）计算得到的，SFE是一个由B2_ResNet和HA组成的模型。
# B2_ResNet是一个深度为50的残差网络，而HA是一个全局注意力机制。
...
# PAM_Module实现了位置注意力机制。
# 它接收输入特征图，并通过查询卷积、键卷积和值卷积来计算注意力图。
# 最后，将值卷积和注意力图相乘并加权相加，得到输出。
...
# Classifier_Module是一个分类器，它接收一个输入特征图并将其通过卷积层处理为具有NoLabels个输出的特征图。
...
# CALayer是通道注意力机制的实现。
# 它使用平均池化来计算特征图的平均值，然后通过卷积层将其缩小和放大，最后将其与原始特征图相乘。
...
# RCAB是残差通道注意力块的实现。
# 它由两个卷积层和一个CALayer组成，它们一起组成了一个残差块。在模型中，它被用作编码器和解码器中的块。
